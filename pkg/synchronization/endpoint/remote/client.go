...
func (c *endpointClient) Scan(ctx context.Context, ancestor *core.Entry, strategy synchronization.ScanStrategy) (*core.Snapshot, error, bool) {
	...
	// by marshaling an ancestor-based snapshot. The endpoint client retains no
	// baseline of its own: the ancestor is retained by the controller for the
	// life of the session anyway, and at steady state its content matches the
	// remote's last snapshot, so a retained baseline would buy only one
	// cycle's worth of delta efficiency. Baseline fidelity affects only delta
	// size, never correctness: the signature and patch below are computed from
	// the same locally generated bytes.
	...
	if err != nil {
		return nil, fmt.Errorf("unable to marshal baseline snapshot: %w", err), false
	}
