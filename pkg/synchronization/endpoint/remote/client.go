...
type endpointClient struct {
	...
	// lastSnapshot is the last snapshot received from the remote endpoint. It
	// is retained (rather than its serialized form) so that the rsync baseline
	// for the next snapshot transfer can be regenerated on demand: the decoded
	// snapshot is already retained by the controller, so this pointer costs
	// nothing, while the serialized form would duplicate tens of megabytes per
	// session. Regeneration relies on deterministic marshaling only for delta
	// efficiency, never for correctness: the signature and patch in Scan are
	// computed from the same locally generated bytes.
...
func (c *endpointClient) Scan(ctx context.Context, ancestor *core.Entry, full bool) (*core.Snapshot, error, bool) {
	...
	// If we have the last received snapshot, then regenerate its serialized
	// form, because it'll be more accurate, but otherwise use the provided
	// ancestor (with some probabilistic assumptions about filesystem behavior).
	var baselineSnapshot *core.Snapshot
	if c.lastSnapshot != nil {
		c.logger.Debug("Using last snapshot as baseline")
		baselineSnapshot = c.lastSnapshot
	} else {
		c.logger.Debug("Using ancestor-based snapshot as baseline")
		baselineSnapshot = &core.Snapshot{
			Content:                ancestor,
			PreservesExecutability: true,
		}
	}
	marshaling := proto.MarshalOptions{Deterministic: true}
	baselineBytes, err := marshaling.Marshal(baselineSnapshot)
	if err != nil {
	...
	// Retain the snapshot so that it can serve as the baseline for receiving
	// the next snapshot, but only do this if the snapshot content was non-nil
	// (i.e. there were entries on disk). If we received a snapshot with no
	// entries, then chances are that it's coming from a remote endpoint that
	// hasn't yet been populated by content, meaning its next transmission
	// (after being populated) is going to be far closer to ancestor than to
	// the empty snapshot that it just sent, and thus we'll want to use the
	// ancestor-based snapshot as the baseline until we receive a populated
	// snapshot.
